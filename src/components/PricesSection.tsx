...
const PricesSection = () => {
  return (
    <section id="prices" className="py-20 bg-foreground text-background">
          ...
          <Accordion type="multiple" className="space-y-3">
            {priceCategories.map((cat, i) => (
              <AccordionItem
                key={i}
                value={`cat-${i}`}
                className="border-2 border-background px-4"
                style={{ boxShadow: "3px 3px 0 hsl(var(--background))" }}
              >
                ...
                <AccordionContent>
                  <div className="space-y-3 pb-2">
                    {cat.items.map((item, j) => (
                      <div
                        key={j}
                        className="flex items-start justify-between gap-4 border-b border-background/20 pb-3 last:border-0"
                      >
                        <div className="flex-1">
                          <span className="font-mono font-bold text-sm text-background">
                            {item.name}
                          </span>
                          <p className="font-mono text-xs text-background/60 mt-0.5">
                            {item.desc}
                          </p>
                        </div>
